    ...
    var body: some View {
        GeometryReader { geometry in
            VStack {
                ...
                .tabViewStyle(PageTabViewStyle(indexDisplayMode: .automatic)) // Enable swipe with page indicator

                // Add padding below the carousel
                Spacer()
                    .frame(height: geometry.size.height * 0.20) // 35% of screen height
            }
        }
        .onAppear {
            // Start any required timers or setup
        }
        .onDisappear {
...
struct LayoutView: View {
    ...
    var body: some View {
        ZStack {
            // Background image for the current layout
            GeometryReader { geometry in
            ...
            VStack {
                Text(player.peripheral?.status?.get_state ?? "Not Connected")
                    .font(.system(size: 20))
                    .padding()
                    .foregroundColor(colorScheme == .dark ? .white : .black)

                Text(player.peripheral?.settings?.description ?? "No Settings")
                    .font(.system(size: 20))
                    .padding()
                    .foregroundColor(colorScheme == .dark ? .white : .black)
                ...
                HStack(spacing: 20) {
                    ...
                    VStack {
                        Button(action: {
                            player.playPausePressedHandler()
                        }) {
